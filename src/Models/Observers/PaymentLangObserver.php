...
namespace WalkerChiu\Payment\Models\Observers;

class PaymentLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.payment.paymentLang')
                ::withTrashed()
        ...
        if (
            config('wk-payment.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-payment.lang_log')
            )
        ) {
